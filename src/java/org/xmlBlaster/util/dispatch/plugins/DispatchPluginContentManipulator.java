 ...
 * </pre>
 * 
 * Normally a client activates this plugin during connection in ConnectQos (see
 * HelloWorldSubscribe.java):
 * 
 * <pre>
 * <connect>
 * <qos> ... 
 ...
 * </pre>
 * 
 * In case ALL clients shall use this plugin, add on server side following line
 * to <code>xmlBlaster.properties</code>:
 * 
 * <pre>
 *   DispatchPlugin/defaultPlugin=ContentManipulator,1.0
 * </pre>
 * 
 * Test example (run commands in four different shells):
 * 
 * <pre>
export XMLBLASTER_HOME=$HOME/xmlBlaster
export CLASSPATH=$XMLBLASTER_HOME/build.tmp/classes:$XMLBLASTER_HOME/lib/xmlBlaster.jar
 * 
java org.xmlBlaster.Main
 * 
 * # launch and hit enter to subscribe ... receives manipulated content:
java javaclients.HelloWorldSubscribe  -session.name Subscriber/1 -dispatchPlugin ContentManipulator,1.0
 * 
 * # launch and hit enter to subscribe ... receives original content:
java javaclients.HelloWorldSubscribe -session.name "Subscriber/2"
 * 
 * # and hit enter to publish next message
java javaclients.HelloWorldPublish -contentMimeExtended "jsonContentToBeFiltered"
 * </pre>
 * 
   ...
   public List<I_Entry> handleNextMessages(I_DispatchManager dispatchManager, List<I_Entry> pushEntries)
      ...
      for (I_Entry ientry : entryList) {
         ...
         log.info("topicId=" + topicId + ",contentMimeExtended=" + contentMimeExtended
               + ": Manipulate content for this subscriber, original=" + contentStr);
         String testContent = this.pluginProperties.getProperty("someTestContent"); // from plugin registration
         contentStr = "Manipulated " + testContent + " random=" + UUID.randomUUID();
