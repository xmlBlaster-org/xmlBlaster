...
public final class QueryQosData extends QosData implements java.io.Serializable, Cloneable
{
   ...
   private PropBoolean subIdGeneratedIncludeClusterNodeId = new PropBoolean(false);
    ...
    * <p>
    * The xmlBlaster checks the given subscriptionId syntax, see SubscriptionInfo#verifySubscriptionId
    * @see <a href="http://www.xmlBlaster.org/xmlBlaster/doc/requirements/engine.qos.subscribe.id.html">The engine.qos.subscribe.id requirement</a>
    */
   ...
   /**
    * Only evaluated in cluster environment and currently only for exact subscriptions with multiSubscribe==false. 
    * <p>
    * Default is false: In case one client connects to multiple failover xmlBlaster cluster nodes,
    * the subId should remain the same, and does not include "/node/myClusterNodeId"<br />
    * Example: "__subId:client/joe/session/1-exact:myTopicId"
    * <p>
    * true: In case different clients attached to several cluster nodes with same sessionName.getRelativeName() exist
    * set this property to true. Else the subId could collide<br />
    * Example: "__subId:/node/heron/client/joe/session/1-exact:myTopicId"
    * <p>
    * Note: Another approach is to let the client control the subId with {@link #setSubscriptionId(String)}
    * @param value
    */
   public void setSubIdGeneratedIncludeClusterNodeId(boolean value) {
      this.subIdGeneratedIncludeClusterNodeId.setValue(value);
   }

   public boolean isSubIdGeneratedIncludeClusterNodeId() {
      return this.subIdGeneratedIncludeClusterNodeId.getValue();
   }
   
   public PropBoolean getSubIdGeneratedIncludeClusterNodeId() {
      return this.subIdGeneratedIncludeClusterNodeId;
   }
    ...
    */
   public String generateSubscriptionId(SessionName sessionName, QueryKeyData subscribeKey, QueryQosData queryQosData) {
      if (sessionName == null) {
      ...
      boolean useCluster = clusterEnabled && clusterNodeId != null && clusterNodeId.length() > 0;
      useCluster = useCluster && queryQosData.isSubIdGeneratedIncludeClusterNodeId();
