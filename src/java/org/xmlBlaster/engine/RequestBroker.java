   ...
   public String subscribe(SessionInfo sessionInfo, QueryKeyData xmlKey, SubscribeQosServer subscribeQos) throws XmlBlasterException   {
      ...
      try {
            ...
            if (vec != null && vec.size() > 0) {
               ...
               for (int i=0; i<vec.size(); i++) {
                  ...
                  if (subscribeQos.getWantInitialUpdate() && xmlKey.isExact()) {
                     ...
                     try {
                        topicHandler = this.glob.getTopicAccessor().access(xmlKey.getOid());
               ...
               log.info("isClusterClient=" + sessionInfo.getConnectQos().isClusterNode()
                   + ",isClusterManagerReady=" + this.glob.isClusterManagerReady()
                   + ",subscribeMultipleClusterForward=" + this.subscribeMultipleClusterForward
                   + " Ignoring duplicate subscription '"
                   +   ((xmlKey.getOid()==null)?((xmlKey.getDomain()==null)?xmlKey.getQueryString():xmlKey.getDomain()):xmlKey.getOid())
                   + "' subId=" + subscriptionId + " from '" + sessionInfo.getSessionName().getAbsoluteName()
                   + "' as you have set multiSubscribe to false"
                   + (subscribeQos.isRecoveredFromPersistenceStore() ? ", recovered from persistenceStore=true" : ""));

               StatusQosData qos = new StatusQosData(glob, sessionInfo.getConnectionQosFormat(), MethodName.SUBSCRIBE);
         ...
         if (qos == null || qos.getSubscriptionId() == null ||  qos.getSubscriptionId().length() < 1) {
            ...
            if (qos == null) qos = new StatusQosData(glob, sessionInfo.getConnectionQosFormat(), MethodName.SUBSCRIBE);
            qos.setSubscriptionId(returnOid);
         }
         propertiesBounced = qos.addClientProperties(SubscribeQos.KEY_BOUNCE_CP, subscribeQos.getData().getClientPropertyArr(), trimPrefix);
   ...
   public MsgUnit[] get(SessionInfo sessionInfo, QueryKeyData xmlKey, GetQosServer getQos) throws XmlBlasterException
   {
      try {
         ...
         if (xmlKey.isRemoteProperties()) { // "__sys__remoteProperties"
            ...
            GetReturnQosServer retQos = new GetReturnQosServer(glob, sessionInfo.getConnectionQosFormat(), null, null);
            ...
            try {
               if (topicHandler.isAlive()) {
                  ...
                  for(int kk=0; kk<msgUnitWrapperArr.length; kk++) {
                     ...
                     GetReturnQosServer retQos = new GetReturnQosServer(glob, sessionInfo.getConnectionQosFormat(), msgUnitWrapper.getMsgQosData(), Constants.STATE_OK);
...
//            	}
         ...
         StatusQosData qos = new StatusQosData(glob, sessionInfo.getConnectionQosFormat(), MethodName.UNSUBSCRIBE);
   ...
   private final String publish(SessionInfo sessionInfo, MsgUnit msgUnit, PublishQosServer publishQos) throws XmlBlasterException
   {
      try {
    	 ...
    	 if (testResponse) {
             log.severe("TESTING CODE IS ACTIVE");
             StatusQosData statRetQos = new StatusQosData(glob, sessionInfo.getConnectionQosFormat(),MethodName.PUBLISH);
         ...
         */
         ...
         if (publishReturnQos == null) {  // assert only
            StatusQosData qos = new StatusQosData(glob, sessionInfo.getConnectionQosFormat(), MethodName.PUBLISH);
   ...
   private String[] erase(SessionInfo sessionInfo, QueryKeyData xmlKey, EraseQosServer eraseQos, boolean isClusterUpdate) throws XmlBlasterException
   {
      try {
         ...
         StatusQosData qos = new StatusQosData(glob, sessionInfo.getConnectionQosFormat(), MethodName.ERASE);
         qos.setState(Constants.STATE_OK);
