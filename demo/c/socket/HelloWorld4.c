...
static void connectionListenerCb(struct XmlBlasterAccess *xa, XBCONSTATE oldState,
	...
	const char *message = (exception == 0 || exception->message == 0) ? ""
			: exception->message;
	printf("[%s] connectionListenerCb transition %s to %s %s %s\n",
			conStateUserData, connectionStateToStr(oldState),
...
int main(int argc, const char* const * argv) {
	...
	for (iarg = 0; iarg < argc; iarg++) {
		if (strcmp(argv[iarg], "-help") == 0 || strcmp(argv[iarg], "--help")
				== 0) {
		...
		if (*xmlBlasterException.errorCode != 0) {
			if (startsWith(xmlBlasterException.errorCode, "user.security")) {
				printf(
						...
						xmlBlasterException.errorCode,
						xmlBlasterException.message);
	...
	{ /* subscribe ... */
		...
		printf(
				"[client4] Subscribe success, returned status is '%s' isOk=%d\n",
	...
	if (true) { /* publishArr */
		...
		if (resp) {
			...
			for (i = 0; i < resp->len; i++) {
				printf(
						"[client4] PublishArr success, returned status is '%s'\n",
	...
	{ /* unSubscribe ... */
		...
		if (resp) {
			...
			for (i = 0; i < resp->len; i++) {
				printf(
						"[client4] Unsubscribe success, returned status is '%s'\n",
						resp->returnQosArr[i].returnQos);
			}
			freeXmlBlasterKey(key);
			freeXmlBlasterQos(qos);
	...
	{ /* get synchronous ... */
		...
		if (*xmlBlasterException.errorCode != 0) {
			printf(
					"[client4] Caught exception in get errorCode=%s, message=%s\n",
					xmlBlasterException.errorCode, xmlBlasterException.message);
			xa->disconnect(xa, 0, &xmlBlasterException);
			freeXmlBlasterAccess(xa);
			exit(EXIT_FAILURE);
		...
		if (msgUnitArr != (MsgUnitArr *) 0) {
			for (i = 0; i < msgUnitArr->len; i++) {
				char *m = messageUnitToXmlLimited(&msgUnitArr->msgUnitArr[i],
						100);
				printf(
						"\n[client4] Get synchronous returned message#%lu/%lu:\n"
							...
							"-------------------------------------\n",
						(unsigned long) (i + 1),
						(unsigned long) msgUnitArr->len, m);
				xmlBlasterFree(m);
			}
			freeXmlBlasterKey(key);
			freeXmlBlasterQos(qos);
