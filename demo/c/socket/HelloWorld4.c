 ...
 * @param userData Is the corresponding XmlBlasterAccess * pointer
...
int main(int argc, const char* const* argv) {
   ...
   printf("[client4] XmlBlaster %s C SOCKET client, try option '-help' if you need"
   ...
   {  /* connect */
      ...
      sprintf(callbackQos,
               ...
               "  </callback>"
               "</queue>",
               callbackSessionId, xa->connectionP->callbackP->hostCB, xa->connectionP->callbackP->portCB);
      sprintf(connectQosStr,
               "<qos>"
      ...
      connectReturnQos = xa->connect(xa, connectQos, myUpdate, &xmlBlasterException);
      if (*xmlBlasterException.errorCode != 0) {
         printf("[client4] Caught exception during connect errorCode=%s, message=%s\n",
                  xmlBlasterException.errorCode, xmlBlasterException.message);
   ...
   { /* ping */
      ...
      if (pingReturn == 0) {
        printf("[client4] ERROR: Pinging a connected server failed: errorCode=%s, message=%s\n",
             xmlBlasterException.errorCode, xmlBlasterException.message);
        freeXmlBlasterAccess(xa);
        exit(EXIT_FAILURE);
      }
   ...
   { /* subscribe ... */
      ...
      if (*xmlBlasterException.errorCode != 0) {
         printf("[client4] Caught exception in subscribe errorCode=%s, message=%s\n",
                  xmlBlasterException.errorCode, xmlBlasterException.message);
   ...
   if (sleepInterval > 0) {
      printf("[client4] Sleeping now and wait on 'HelloWorld' updates (start a publisher somewhere) ...\n");
   ...
   {  /* publish ... */
      ...
      if (*xmlBlasterException.errorCode != 0) {
         printf("[client4] Caught exception in publish errorCode=%s, message=%s\n",
                  xmlBlasterException.errorCode, xmlBlasterException.message);
         xa->disconnect(xa, 0, &xmlBlasterException);
         freeXmlBlasterReturnQos(returnQos);
         freeXmlBlasterAccess(xa);
   ...
   if (true) {  /* publishArr */
      ...
      printf("[client4] Publishing messages 'HelloWorld0' and 'HelloWorld1' ...\n");
      holder.len = 2;
      ...
      if (*xmlBlasterException.errorCode != 0) {
         printf("[client4] Caught exception in publishArr errorCode=%s, message=%s\n",
                  xmlBlasterException.errorCode, xmlBlasterException.message);
         xa->disconnect(xa, 0, &xmlBlasterException);
   ...
   if (true) {  /* publishOneway */
      MsgUnitArr holder;
      memset(&holder, 0, sizeof(MsgUnitArr));
      printf("[client4] Publishing oneway messages 'HelloWorld0' and 'HelloWorld1' ...\n");
      holder.len = 2;
      holder.msgUnitArr = (MsgUnit *)calloc(holder.len, sizeof(MsgUnit));
      holder.msgUnitArr[0].key = strcpyAlloc("<key oid='HelloWorld0'/>");
      holder.msgUnitArr[0].content = strcpyAlloc("Some message payload");
      ...
      freeMsgUnitArrInternal(&holder);
      if (*xmlBlasterException.errorCode != 0) {
         printf("[client4] Caught exception in publishOneway errorCode=%s, message=%s\n",
                  xmlBlasterException.errorCode, xmlBlasterException.message);
         xa->disconnect(xa, 0, &xmlBlasterException);
         freeXmlBlasterAccess(xa);
         exit(EXIT_FAILURE);
   ...
   {  /* unSubscribe ... */
      ...
      if (resp) {
         ...
         printf("[client4] Caught exception in unSubscribe errorCode=%s, message=%s\n",
                  xmlBlasterException.errorCode, xmlBlasterException.message);
         xa->disconnect(xa, 0, &xmlBlasterException);
         freeXmlBlasterKey(key);
         freeXmlBlasterQos(qos);
   ...
   {  /* erase ... */
      ...
      if (*xmlBlasterException.errorCode != 0) {
         printf("[client4] Caught exception in erase errorCode=%s, message=%s\n",
                  xmlBlasterException.errorCode, xmlBlasterException.message);
         xa->disconnect(xa, 0, &xmlBlasterException);
         freeXmlBlasterAccess(xa);
         exit(EXIT_FAILURE);
   ...
   if (xa->disconnect(xa, 0, &xmlBlasterException) == false) {
      printf("[client4] Caught exception in disconnect, errorCode=%s, message=%s\n",
               xmlBlasterException.errorCode, xmlBlasterException.message);
      freeXmlBlasterAccess(xa);
      exit(EXIT_FAILURE);
   }
