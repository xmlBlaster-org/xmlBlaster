/*------------------------------------------------------------------------------
Name:      TestActivateDispatcher.java
 ...
 * Invoke examples:<br />
 * <pre>
 *   java junit.textui.TestRunner -noloading org.xmlBlaster.test.client.TestActivateDispatcher
 *   java junit.swingui.TestRunner -noloading org.xmlBlaster.test.client.TestActivateDispatcher
 * </pre>
 */
public class TestActivateDispatcher extends TestCase implements I_Callback
{
   private static String ME = "TestActivateDispatcher";
   ...
   public TestActivateDispatcher(String testName) {
      this(null, testName);
   }

   public TestActivateDispatcher(Global glob, String testName) {
      super(testName);
    ...
    * TEST: <br />
    */
   public void testDispatchActive() {
      try {
         String oid = "TestDispatchActive";
         doPublish(1, oid);
         doSubscribe(oid);
         doPublish(2, oid);
         doPublish(3, oid);
         log.info(ME, "Going to publish 3 times on message '" + oid + "'");
         assertEquals("wrong number of updates received", 3, this.updateInterceptor.count());
         this.updateInterceptor.clear();

         String getOid = "__cmd:client/" + this.sessionName + "/?dispatcherActive";
         ...
         for (int i=0; i < msg.length; i++) {
            this.log.info(ME, "testDispatchActive: dispatcherActive: (" + i + ") : '" + msg[i].getContentStr() + "'");
         ...
         assertEquals("wrong number of messages returned", 1, msg.length);
         for (int i=0; i < msg.length; i++) {
            this.log.info(ME, "testDispatchActive: dispatcherActive (result): (" + i + ") : '" + msg[i].getContentStr() + "'");
   ...
   /**
    * Invoke: java org.xmlBlaster.test.client.TestActivateDispatcher
    * <p />
   ...
   public static void main(String args[])
   {
      ...
      TestActivateDispatcher testSub = new TestActivateDispatcher(glob, "TestActivateDispatcher/1");

      testSub.setUp();
      testSub.testDispatchActive();
      testSub.tearDown();
   }
}
