...
static Dll_Export void freeQueueEntryArr(QueueEntryArr *queueEntryArr);
static Dll_Export void freeQueueEntryArrInternal(QueueEntryArr *queueEntryArr);
static Dll_Export void freeQueueEntryData(QueueEntry *queueEntry);
static Dll_Export void freeQueueEntry(QueueEntry *queueEntry);
static Dll_Export char *queueEntryToXmlLimited(QueueEntry *queueEntry, int maxContentDumpLen);
...
static void persistentQueuePut(I_Queue *queueP, QueueEntry *queueEntry, QueueException *exception)
{
   ...
   while (continueTrying) {
    retval = sqlite_exec(dbInfo->db, sqlQuery, callback, 0, &msg);
    switch (retval) {
      case SQLITE_BUSY:
      ...
      while (!done) {
         ...
         switch( rc ){
            ...
            case SQLITE_ROW:
               ...
               */
               queueEntry = calloc(1, sizeof(QueueEntry));
               ...
               numAssigned = sscanf(pazValue[6], "%d", &queueEntry->embeddedBlob.dataLen);
               ...
               decodeSize = sqlite_decode_binary(pazValue[7], (unsigned char *)queueEntry->embeddedBlob.data);
               ...
               freeQueueEntry(queueEntry); // !!! TODO: Return it
            break;
            case SQLITE_ERROR:   /* If exists already */
            case SQLITE_MISUSE:
            default: /* case SQLITE_ERROR: case SQLITE_MISUSE: */
...
static Dll_Export char *queueEntryToXmlLimited(QueueEntry *queueEntry, int maxContentDumpLen)
{
   ...
   if (maxContentDumpLen == 0)
      *contentStr = 0;
   else if (maxContentDumpLen > 0 && (size_t)maxContentDumpLen < (queueEntry->embeddedBlob.dataLen-5))
...
int main(int argc, char **argv) {
   ...
   queueEntry.embeddedBlob.data = "012345678901";
