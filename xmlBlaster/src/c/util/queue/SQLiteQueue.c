...
Dll_Export void freeQueue(I_Queue *queueP)
{
   ...
   sqlite *db = queueP->privateObject;
   if (db) {
      sqlite_close(db);
...
static void persistentQueueInitialize(I_Queue *queueP, QueueException *exception)
{
   ...
   db = sqlite_open(dbname, OPEN_RW, &errMsg);
   queueP->privateObject = db;
   ...
   if (queueP->log) queueP->log(__FILE__, "initialize(%s) successful", dbname);
}

static void persistentQueuePut(I_Queue *queueP, QueueEntry *queueEntry, QueueException *exception)
{
   if (queueP->log) queueP->log(__FILE__, "put() of %ld done", queueEntry->uniqueId);
}

static QueueEntryArr *persistentQueuePeekWithSamePriority(I_Queue *queueP, int32_t maxNumOfEntries, int64_t maxNumOfBytes, QueueException *exception)
{
   if (queueP->log) queueP->log(__FILE__, "peekWithSamePriority() of max %d entries done", (int)maxNumOfEntries);
   return 0;
}
...
int main(int argc, char **argv) {
   ...
   queueEntry.embeddedBlob.dataLen = 10;
