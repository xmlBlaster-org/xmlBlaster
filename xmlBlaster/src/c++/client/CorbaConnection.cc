...
Version:   $Id: CorbaConnection.cc,v 1.1 2000/07/06 22:55:44 laghi Exp $
...
namespace client {
   ...
   CosNaming::NamingContext_ptr CorbaConnection::getNamingService() {
      if (log_.CALLS) log_.calls(me(), "getNamingService() ...");
   ...
   authenticateIdl::AuthServer_ptr 
      ...
      if (log_.CALLS) log_.calls(me(), "getAuthenticationService() ...");
   ...
   serverIdl::Server_ptr
      ...
      if (log_.CALLS) log_.calls(me(), "login(" + loginName + ") ...");
      if ( !CORBA::is_nil(xmlBlaster_ /*.in()*/ )) {
   ...
   void CorbaConnection::loginRaw() {
      if (log_.CALLS) log_.calls(me(),"loginRaw(" + loginName_ + ") ...");
      try {
   ...
   bool CorbaConnection::logout() {
      if (log_.CALLS) log_.calls(me(), "logout() ...");
      
      if ( CORBA::is_nil(xmlBlaster_ /*.in()*/ )) {
	 log_.warning(me(), "No logout, you are not logged in");
	 return false;
      }
      else log_.warning(me(), "Logout!");
   ...
   string CorbaConnection::subscribe(const string &xmlKey, const string &qos) {
      if (log_.CALLS) log_.calls(me(), "subscribe() ...");
   ...
   void CorbaConnection::unSubscribe(const string &xmlKey, 
				     const string &qos) {
      if (log_.CALLS) log_.calls(me(), "unSubscribe() ...");
...
//        }
   ...
   serverIdl::StringArr*
      ...
      if (log_.CALLS) log_.calls(me(), "publishArr() ...");
   ...
   serverIdl::StringArr* 
      ...
      if (log_.CALLS) log_.calls(me(), "erase() ...");
   ...
   serverIdl::MessageUnitArr*
      ...
      if (log_.CALLS) log_.calls(me(), "get() ...");
