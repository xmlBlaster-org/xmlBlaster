...
public class CbServerPluginManager extends PluginManagerBase
{
   ...
   public CbServerPluginManager(Global glob) {
      ...
      if (log.CALL) log.call(ME, "Constructor CbServerPluginManager");
   }
   ...
   public I_CallbackServer getPlugin(String type, String version) throws XmlBlasterException {
      ...
      I_CallbackServer driver = (I_CallbackServer)super.instantiatePlugin(pluginInfo);
      return driver;
   }
   ...
   public void activateDrivers() throws XmlBlasterException {
      if (log.TRACE) log.trace(ME, "Don't know how to activate the calback server protocol drivers, they are created for each client and session separately");
   }

   public final void deactivateDrivers(boolean force) {
      if (log.TRACE) log.trace(ME, "Don't know how to deactivate the calback server protocol drivers, they are created for each client and session separately");
   }

   public void shutdownDrivers(boolean force) throws XmlBlasterException {
      if (log.TRACE) log.trace(ME, "Don't know how to shutdown the calback server protocol drivers, they are created for each client and session separately");
   }
}
