...
public class Receiver implements I_Plugin, I_Callback {
   ...
   public Receiver(Global globOrig, String name, PluginInfo info) throws XmlBlasterException {
      ...
      try {
         this.callback = new FileWriterCallback(directoryName, lockExtention, overwrite);
      }
      catch (Exception ex) {
   ...
   public synchronized String update(String cbSessionId, UpdateKey updateKey, byte[] content, UpdateQos updateQos) throws XmlBlasterException {
      try {
         ...
         ex.printStackTrace();
         throw new XmlBlasterException(this.global, ErrorCode.USER_CLIENTCODE, "Receiver.update", "user exception", ex);
      }
      catch (Throwable ex) {
         ex.printStackTrace();
         throw new XmlBlasterException(this.global, ErrorCode.USER_CLIENTCODE, "Receiver.update", "user throwable", ex);
      }
   }
}
