...
public class XBTopicSubscriber implements TopicSubscriber, I_Callback {
   ...
   XBTopicSubscriber(I_XmlBlasterAccess access, Topic topic, String msgSelector, boolean noLocal) {
      this.access = access;
   ...
   synchronized public String update(String cbSessionId, UpdateKey updateKey, byte[] content, UpdateQos updateQos) throws XmlBlasterException {
      try {
         if (this.msgListener != null) {
            ...
            throw new XmlBlasterException(this.global, ErrorCode.USER_UPDATE_ERROR, ME + ".update: the message listeer has not been assigned yet");         
         }
      }
      catch (Throwable ex) {
