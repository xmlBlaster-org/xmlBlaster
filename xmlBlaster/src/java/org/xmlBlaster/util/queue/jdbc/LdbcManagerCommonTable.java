/*------------------------------------------------------------------------------
Name:      LdbcManagerCommonTable.java
...
/**
 * Delegate class which takes care of SQL specific stuff for the LdbcQueuePlugin
 * class.
 * <p>
 ...
 *  blob
 *
 * The tables needed for each LdbcManagerCommonTable instance are the following:
 ...
 * nodesTableName
 * 
 * @see <a href="http://www.xmlBlaster.org/xmlBlaster/doc/requirements/queue.jdbc.ldbc.html">The queue.jdbc.ldbc requirement</a>
 * @author <a href="mailto:xmlBlaster@marcelruff.info">Marcel Ruff</a>
 * @author <a href='mailto:laghi@swissinfo.org'>Michele Laghi</a>
 * @author <a href='mailto:bencom@bencom.co.nz'>Peter Bennett</a>
 *
 */
public class LdbcManagerCommonTable implements I_StorageProblemListener, I_StorageProblemNotifier {

   private static final String ME = "LdbcManagerCommonTable";
   private final Global glob;
   private final LogChannel log;
   private final LdbcConnectionPool pool;
   ...
   private String longintTxt = "decimal(19,0)";
   ...
   /**
    * @param LdbcConnectionPool the pool to be used for the connections to
    *        the database. IMPORTANT: The pool must have been previously
    *        initialized.
    */
   public LdbcManagerCommonTable(LdbcConnectionPool pool, I_EntryFactory factory)
...
//      this.blobVarName = (String)names.get("blobVarName");
      if (this.blobVarName == null) this.blobVarName = "ablob";

//      this.keyAttr = (String)names.get("keyAttr");
      if (this.keyAttr == null) this.keyAttr = "not null"; // could be "not null" for MySQL
...
*/
   ...
   */
   ...
   public final boolean addNode(String nodeId) throws XmlBlasterException {
      ...
      boolean ret = false;
      try {
         query = new LdbcPreparedQuery(pool, req, !AUTO_COMMIT, this.log, -1);
         query.rs.next();         
...
*/
   ...
   public final boolean removeNode(String nodeId) throws XmlBlasterException {
      ...
      synchronized (this.nodesCache) {
         ...
         LdbcPreparedQuery query = null;
         try {
            query = new LdbcPreparedQuery(pool, req, false, this.log, -1);
            query.rs.next();         
            int size = query.rs.getInt(1);
...
//            exStatement.setQueryTimeout(this.pool.getQueryTimeout());
   ...
   public final int cleanUp(String queueName, String nodeId) throws XmlBlasterException {
      ...
      try {
	 String req = "delete from " + this.entriesTableName + " where queueName='" + queueName + "' AND nodeId='" + nodeId + "'";
         if (this.log.TRACE)
            this.log.trace(getLogId(queueName, nodeId, "cleanUp"), " request is '" + req + "'");
         update(req);     
         req = "delete from " + this.queuesTableName + " where queueName='" + queueName + "' AND nodeId='" + nodeId + "'";
         if (this.log.TRACE)
            this.log.trace(getLogId(queueName, nodeId, "cleanUp"), " request is '" + req + "'");
         return update(req);
      }
...
//            entries.add(this.factory.createEntry(prio, dataId, typeName, persistent, sizeInBytes, blob, storageId));
    ...
    * @return LdbcConnectionPool the pool managing the connections to the DB
    */
   public LdbcConnectionPool getPool() {
      return this.pool;
   }
...
//      int count = 0;
  ...
  //    ArrayList list = null;
   ...
   public ArrayList getEntriesWithLimit(StorageId storageId, String nodeId, I_Entry limitEntry)
      ...
      LdbcPreparedQuery query = null;
      try {
         query = new LdbcPreparedQuery(pool, req, this.log, -1);
   ...
   public final long getNumOfEntries(String queueName, String nodeId)
      ...
      try {
         query = new LdbcPreparedQuery(pool, req, true, this.log, -1);
         query.rs.next();
   ...
   public static void wipeOutDB(Global glob, String confType, String confVersion, java.util.Properties properties, boolean setupNewTables) 
      ...
      LdbcConnectionPool pool = new LdbcConnectionPool();
      try {
      ...
      if ("org.xmlBlaster.util.queue.jdbc.JdbcQueuePlugin".equals(queueClassName)) {
         ...
         LdbcManagerCommonTable manager = new LdbcManagerCommonTable(pool, null);
   ...
   public static void main(String[] args) {
      ...
      if ((type == null) || (version == null)) {
         System.out.println("usage: java org.xmlBlaster.util.queue.jdbc.LdbcManagerCommonTable -wipeout.pluginType JDBC -wipeout.pluginVersion 1.0");
         System.exit(1);
      }

      try {
         LdbcManagerCommonTable.wipeOutDB(glob, type, version, null, false);
      }
      catch (Exception ex) {
