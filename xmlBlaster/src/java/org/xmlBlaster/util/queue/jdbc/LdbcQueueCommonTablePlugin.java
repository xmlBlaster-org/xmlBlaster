/*------------------------------------------------------------------------------                              
Name:      LdbcQueueCommonTablePlugin.java
...
public final class LdbcQueueCommonTablePlugin implements I_Queue, I_StoragePlugin, I_Map
{
   ...
   private LdbcManagerCommonTable manager = null;
...
*/
   ...
   /**
    * Returns a LdbcManagerCommonTable for a specific queue. It strips the queueId to
    ...
    */
   protected LdbcManagerCommonTable getLdbcQueueManagerCommonTable(PluginInfo pluginInfo)
      ...
      LdbcManagerCommonTable manager = null;
      if (obj == null) {
         synchronized (LdbcManagerCommonTable.class) {
            ...
            if ( obj == null) {
               ...
               try {
                  ...
                  manager = new LdbcManagerCommonTable(pool, this.glob.getEntryFactory(managerName));
                  pool.registerStorageProblemListener(manager);
                  manager.setUp();
                  if (log.TRACE) log.trace(ME, "Created LdbcManagerCommonTable instance for storage plugin configuration '" + managerName + "'");
               }
      ...
      else manager = (LdbcManagerCommonTable)obj;

      try {
         if (!manager.getPool().isInitialized()) {
            manager.getPool().initialize(this.glob, pluginInfo.getParameters());
            if (log.TRACE) log.trace(ME, "Initialized LdbcManager pool for storage class '" + managerName + "'");
         }
      }
      catch (ClassNotFoundException ex) {
         throw new XmlBlasterException(this.glob, ErrorCode.RESOURCE_DB_UNAVAILABLE, location, "getLdbcQueueManager: class not found when initializing the connection pool", ex);
      }
      catch (SQLException ex) {
         throw new XmlBlasterException(this.glob, ErrorCode.RESOURCE_DB_UNAVAILABLE, location, "getLdbcQueueManager: sql exception when initializing the connection pool", ex);
      }
      catch (Throwable ex) {
         if (this.log.TRACE) this.log.trace(location, "getJdbcCommonTableQueueManager internal exception: " + ex.toString());
         throw new XmlBlasterException(this.glob, ErrorCode.INTERNAL_UNKNOWN, location, "getLdbcQueueManager: throwable when initializing the connection pool", ex);
      }
      ...
      throws XmlBlasterException
   {
      if (this.isDown) {
         ...
         this.manager = getLdbcQueueManagerCommonTable(this.pluginInfo);
...
*/  
   ...
   public LdbcManagerCommonTable getManager() {
      return this.manager;
   }
   ...
   public final String toXml(String extraOffset) {
      ...
      sb.append(offset).append("<LdbcQueueCommonTablePlugin id='").append(getStorageId().getId());
      ...
      sb.append(offset).append("</LdbcQueueCommonTablePlugin>");
      return sb.toString();
   }
