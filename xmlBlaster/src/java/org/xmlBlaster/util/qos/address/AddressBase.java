...
public abstract class AddressBase
{
   ...
   public final String usage()
   {
    ...
    //text += "   -queue.onFailure    Error handling when connection failed (after all retries etc.) [" + CbQueueProperty.DEFAULT_onFailure + "].\n";
      text += "   -dispatch/clientSide/burstMode/collectTimeOneway   Number of milliseconds we shall collect oneway publish \n";
      text += "                                      messages [" + Address.DEFAULT_collectTime + "].\n";
    ...
    //text += "   -oneway             Shall the publish() messages be send oneway (no application level ACK) [" + Address.DEFAULT_oneway + "]\n";
      text += "   -dispatch/clientSide/pingInterval  Pinging every given milliseconds [" + getDefaultPingInterval() + "]\n";
      text += "   -dispatch/clientSide/retries       How often to retry if connection fails (-1 is forever) [" + getDefaultRetries() + "]\n";
      text += "   -dispatch/clientSide/delay         Delay between connection retries in milliseconds [" + getDefaultDelay() + "]\n";
    ...
    //text += "   -compress.minSize   Messages bigger this size in bytes are compressed [" + Address.DEFAULT_minSize + "]\n";
      ...
      text += "   -dispatch/callback/sessionId       The session ID which is passed to our callback server update() method.\n";
      text += "   -dispatch/callback/burstMode/collectTime  Number of milliseconds xmlBlaster shall collect callback messages [" + CallbackAddress.DEFAULT_collectTime + "].\n";
      text += "                                      The burst mode allows performance tuning, try set it to 200.\n";
      text += "   -dispatch/callback/oneway          Shall the update() messages be send oneway (no application level ACK) [" + CallbackAddress.DEFAULT_oneway + "]\n";
      text += "   -dispatch/callback/pingInterval    Pinging every given milliseconds [" + getDefaultPingInterval() + "]\n";
      text += "   -dispatch/callback/retries         How often to retry if callback fails (-1 forever, 0 no retry, > 0 number of retries) [" + getDefaultRetries() + "]\n";
      text += "   -dispatch/callback/delay           Delay between callback retries in milliseconds [" + getDefaultDelay() + "]\n";
      text += "   -dispatch/callback/compress.type   With which format message be compressed on callback [" + CallbackAddress.DEFAULT_compressType + "]\n";
      ...
      text += "   -dispatch/callback/protocol        You can choose another protocol for the callback server [defaults to -client.protocol]\n";
