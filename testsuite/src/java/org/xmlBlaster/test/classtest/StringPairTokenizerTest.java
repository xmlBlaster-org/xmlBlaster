 ...
 * org.xmlBlaster.test.classtest.StringPairTokenizerTest
 */
public class StringPairTokenizerTest extends TestCase {

	final static String ME = "StringPairTokenizerTest";
	protected Global glob;
	private static Logger log = Logger.getLogger(StringPairTokenizerTest.class
			.getName());
	int counter = 0;
	...
	public void testTokenizerApos1() {
		{
	...
	public void testEscape() {
		{
			// &comma; escapes are not supported
			String csv = "v=1&comma;2";
			String[] toks = StringPairTokenizer.parseLine(new String[] { csv },
					',', '"', false, true);
			assertEquals(1, toks.length);
			assertEquals("v=1&comma;2", toks[0]);
		}
		{
			String csv = "k=1,v=\"1,2\"";
			String[] toks = StringPairTokenizer.parseLine(new String[] { csv },
					',', '"', false, true);
			assertEquals(2, toks.length);
			assertEquals("k=1", toks[0]);
			assertEquals("v=\"1,2\"", toks[1]);
		}
		{
			String csv = "k=1,v=\"1,2\"";
			String[] toks = StringPairTokenizer.parseLine(new String[] { csv },
	...
	public void testClientProperties() {
		try {
			...
			Map<String, ClientProperty> props = StringPairTokenizer
					.parseToStringClientPropertyPairs(cmd, "&", "=");
	...
	public void testURL() {
		// TODO: need to add " around some parameter-values and extend the
		...
		try {
			...
			// assertEquals("wrong value", "action",
			// props.get("initReplication"));
	...
	public void testMapCSV() {
		{
			...
			assertEquals("a value with \" apost, and semicolon",
					(String) map.get("aKey"));
	...
	public static void main(String args[]) {
		StringPairTokenizerTest test = new StringPairTokenizerTest(
				"StringPairTokenizerTest", args);
